...
export const assessAnswerAgainstQuestionService = async ({
  ...
  const userPrompt = `
You review whether a forum ANSWER draft addresses the QUESTION (relevance and completeness of engagement - not grading).
...
Rules:
Level: "strong" if the draft clearly engages with the question; "partial" if somewhat related but missing key parts of the ask; "weak" if vague or mostly unrelated.
...
`;
  ...
  try {
    ...
    console.error("Assess answer against question service error:", error);
...
export const generateQuestionDraftCoachService = async ({ title, content }) => {
  const userPrompt = `
You help learners write clearer technical forum posts.
