...
// *Entry point for users to login

export default function Auth() {
  const navigate = useNavigate();
  const location = useLocation();
  const { register, login } = useAuth();
  // to toggle between form
  const [isLogin, setIsLogin] = useState(true);

  // Registration form state:hold input fields
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        // Delay redirect to show successful message before jumping to Dashboard
